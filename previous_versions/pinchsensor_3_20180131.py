...
def get_data():
    global sample_size, stop_plot, time_buffer, voltage_buffer, current_recording, trigger_buffer, testing, rescaleX
    try:
        time.sleep(2)
        ser.flushInput()
        ...
        while 1:
            ...
            if stop_plot:
                threading.Thread(target=write_serial, args=["stop_plot"]).start()
                break
...
class Ui_pinchSensorUI(object):
    ...
    def plot(self):
        ...
        stop_plot = not stop_plot
        if stop_plot:
            current_recording += 1
            self.buttonPlot.setText("Start Plot")

            self.buttonPlot.setDisabled(True)
            QtCore.QTimer.singleShot(2000, self.enable_plot_button)
    ...
    def updater(self):
        ...
        if testing == 1:
            ...
            self.plotItem.addLine(x=time_buffer[-1]).setPen(pg.mkPen(color=(0, 0, 255)))
    ...
    def clear_plot(self):
        ...
        for trigger in last_triggers:
            self.plotItem.addLine(x=trigger).setPen(pg.mkPen(color=(0, 0, 255)))
